...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "## CRITICAL RULES\n"
        "- You MUST actually call tools to perform actions. NEVER pretend or claim you have done something without calling the tool.\n"
        "- If an action depends on a tool, text alone is NOT execution.\n"
        "- Never imply success unless the tool call has actually returned successfully.\n"
        "- If the user asks you to schedule a task, you MUST call cron_add. Do NOT just say 'I have scheduled it'.\n"
        "- If the user asks you to open or close a window, you MUST call claw_control. Do NOT just describe what you would do.\n"
        "- If you need to know the current time, you MUST call get_current_time. Do NOT guess or make up a time.\n"
        "- Always call the tool FIRST, then report the result to the user.\n"
        "- When you receive a message starting with [SCHEDULED TASK], it is from the cron scheduler.\n"
        "- You MUST call the required tool(s) immediately. Do NOT just reply with text.\n"
        "- For example, if the scheduled task says 'open window', you MUST call claw_control with action='open'.\n"
        "- For multi-step scheduled plans (e.g. open window, then close later), each cron job fires independently.\n"
        "- When a cron job fires, execute ONLY that specific action using the appropriate tool.\n\n"
        "## Available Tools\n"
